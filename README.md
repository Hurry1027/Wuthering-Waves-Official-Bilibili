...
### 6. 程序转换流程
- 步骤：扫描版本-执行备份(需要确认)-执行转换(需要确认) ：
  1. 运行 switching.py 后，程序将按照以下步骤执行：
  2. 扫描当前版本 ：程序会调用 scan.py 扫描当前版本目录，并将结果保存到 Bilibili.json 文件中，程序会调用 structured_diff.py 生成差异文件，将差异信息保存到 structured_diff.json 文件中。
  3. 执行备份(可跳过) ：程序会根据差异文件执行备份操作，并将备份文件保存到 backup 目录中，一般在客户端更新后再选择执行备份，平时选择否就行了。
  4. 生成转换计划 ：程序会根据差异文件生成转换计划，并将计划保存到 conversion_plan.txt 文件中，你可以查看程序会复制和覆盖的文件。
  5. 执行转换计划 ：程序会根据转换计划执行版本转换操作。
  6. 完成转换 ：程序将提示转换完成，并打开相应客户端。

###  7. 大版本更新（2.3更新）
  1. 预下载期间，比如先在B服预下载，那么转换到官服时，选择备份时，注意备份的文件大小，备份文件若包含预下载文件，则不需要备份，正常切换官服即可。
  2. 版本更新当天，请先切换到B服客户端点击更新按钮进行更新。
  3. （重点）转换到官服时，进行扫描，但不备份不转换，然后请手动转换及修复游戏文件：
     - 运行本程序，选择备份文件并转换，让程序扫描当前B服客户端所有文件，提示结果已保存至 Bilibili.json
     - 选择**不进行备份**
     - 选择**不进行转换**
     - 关闭本程序
     - 打开官服客户端，点击修复文件，等待客户端修复完成
     - 手动更改本程序目录下的sync_state.json中的current_ver为Official
     - 此时已经手动从B服切换到了官服版本。
  4. （重点）再次转换到B服时，进行扫描，进行备份，但不执行转换，然后再一次手动转换及修复游戏文件：
      - 运行本程序，选择备份文件并转换，让程序扫描当前官服客户端所有文件，提示结果已保存至 Official.json
     - 选择**进行备份**
     - 选择**不进行转换**
     - 关闭本程序
     - 打开B服客户端，点击修复文件，等待客户端修复完成
     - 手动更改本程序目录下的sync_state.json中的current_ver为Bilibili
     - 此时已经手动从B服切换到了官服版本。
  5. （重点）再次转换到官服时，进行扫描，进行备份，然后执行转换，至此大版本更新完成。
  6. 总结：大版本更新中，进行了两次手动转换及客户端修复文件。
